...
const Articles =()=>{
    ...
    return (
        <List
        size="large"
        header={<div style={{fontSize: "32px"}}>{catagory} <Button onClick={()=>navigate(`/clubmain/${clubName}/createarticle`)} icon={<EditOutlined />}>새 글 쓰기</Button></div>}
        pagination={{
            onChange: (page) => {
              console.log(page);
            },
        ...
        dataSource={articles}
        renderItem={(item) => <List.Item onClick={()=>selectArticle(item.id)} style={{border:"1px solid gray"}}>title{item.title}  nickname{item.User.nickname}</List.Item>}
        />
    );    
};
