...
exports.post_nft_deploy = async (req, res, next) => {
  try {
    const { club_id, nft_name, nft_symbol, nft_desc, nft_price, deploy_count } = req.body;
    ...
    if(result_deploy.msg === "success") {
      ...
      await service.setNFTDeploy(club_id, meta_cid, nft_price, deploy_count, result_deploy.value); //<<<< Database 함수 추가 필요   
    ...
    try {
      ...
      if(result_trans === "ok") { 
        ...
        if(result_supply === "ok") { 
          ...
          const return_mint = await contract_proc.MintNFT(address, result_db.contract_add, result_db.token_URI);
          if(return_mint === "success") {
            // minting을 한 user 계정의 NFT 정보를 database에 insert 하는 함수 구현 필요
            await service.setNFTMint(address, return_mint.value, club_id);  //<<<< Database 함수 추가 필요   
...
exports.post_nft_parts_upload = async (req, res, next) => {
  try {
    ...
    const result = await file_proc.filesMove(process.env.NFT_UPLOAD_PATH, targetDir);
    if(result === "success") {
