...
exports.testKIP17 = async () => {
  ...
  const kip17Instance = new caver.klay.KIP17(
    process.env.KIP17_CONTRACT_ADDRESS
  );
  ...
  const res = await kip17Instance.mintWithTokenURI(
    "0x723c3659772Fe80284793C6a20bff9071bc683F6",
    Number(num) + 1,
    tokenURI,
...
exports.kip7_Deploy = async () => {
  try {
    ...
    if (typeof account_info === "undefined") {
      await caver.klay.accounts.wallet.add(
        process.env.ACCOUNT_SECRET_KEY,
        process.env.SERVER_ACCOUNT
      );
    }
    ...
    if (typeof result === "undefined") {
      ...
      env.setEnvValue(
        "KIP7_CONTRACT_ADDRESS",
        deployedInstance.options.address
      );
      console.log(env.getEnvValue("KIP7_CONTRACT_ADDRESS"));
      return deployedInstance.options.address;
    } else {
...
exports.getTokenTransCheck = async (address, tx_hash, nft_price) => {
  try {
    ...
    const result = await caver_kas.kas.tokenHistory.getTransferHistoryByTxHash(
      tx_hash
    );
